...
//        ArrayList<String> horses = new ArrayList<String>(Arrays.asList("t3", "t2", "t1"));
//        Lesson7_1.permutate(horses, new ArrayList<String>());

        char[] passwordCharts = {'a', 'b', 'c', 'd', 'e'};
        // getPassword(passwordCharts,4,""); 206896607

        int[] base = new int[]{1, 2, 5, 10};
        int[] record = new int[35];
        ArrayList<String> arrayList = new ArrayList<>();
        findSuit(record, base, arrayList, 0, 35, 0);
    ...
    public static void createPwds(String[] record, String[] bases, List<String> result, int index) {
        for (int i = index; i < record.length; i++) {
            for (int j = 0; j < bases.length; j++) {
                record[i] = bases[j];
                if (j + 1 != bases.length) {
                    createPwds(record, bases, result, index + 1);
                }
            }
        }
        result.add(Arrays.toString(record));
    }

    static int k = 0;
    public static void findSuit(int[] record, int[] bases, List<String> result, int index, int total, int nextSum) {
        if (index == record.length) return;
        for (int j = 0; j < bases.length; j++) {

            record[index] = bases[j];
            nextSum += bases[j];
            if (nextSum == total) {
                Arrays.copyOf(record, index + 1);
                return;
            } else if (nextSum > total) {
                return;
            } else {
                k++;
                findSuit(record, bases, result, index + 1, total, nextSum);
            }
            nextSum -= bases[j];
        }
    }
